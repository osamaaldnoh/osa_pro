...
import '../../domain/repositories/item_units_repositories.dart';
...
class ItemUnitsRepositoryImp implements ItemUnitsRepository {
      ...
      if (localItemUnits.isNotEmpty) {
        ...
        if (await netWorkInfo.isConnected) {
          try {
            ...
            remoteItemUnits.forEach((value) {
              localDataSources.saveItemUnit(value);
            });
  ...
  @override
  Future<Either<Failure, List<ItemUnitsEntity>>> getUnitsWhereItem(
      int itemId) async {
    try {
      final response = await localDataSources.getUnitsWhereItemId(itemId);
